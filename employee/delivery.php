    ...
    while($gherin=mysqli_fetch_array($gun)){
        ...
        while($getout=mysqli_fetch_array($hatt)){
            ...
            <tr>
                ...
                <td align=center><button style="height:40px;width:90px;"><a href="delivery.php?pro_id=<?php echo $product_id; ?>" style="text-decoration:none;color:black;">delivered</a></button> </td>
            </tr>
...
if(isset($_GET['pro_id'])){
    ...
    $js="UPDATE `pending_orders` SET `order_status`='$newstatus' WHERE product_id='$rpro'";
    $rgh=mysqli_query($conn, $js);

    $bem="SELECT * from pending_orders";
    $ret=mysqli_query($conn, $bem);
    $ewr=mysqli_fetch_array($ret);
    $status=$ewr['order_status'];


    $js="UPDATE `customer_order` SET `order_status`='$newstatus' WHERE product_id='$rpro'";
    $rgh=mysqli_query($conn, $js);
